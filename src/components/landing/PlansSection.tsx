    ...
    <section className="relative pb-28 pt-16 sm:pb-36 sm:pt-28 md:py-40 overflow-hidden bg-[#FF002F] text-white">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10 my-4 sm:my-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-5 bg-white rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-9 shadow-2xl space-y-4 sm:space-y-5 text-center lg:text-left text-black z-30">
            <h2 className="font-fraunces text-2xl sm:text-4xl lg:text-5xl font-bold leading-[1.2] text-black tracking-tight pb-1">
              every kind of plan{" "}
              <span className="text-primary font-bold block md:inline lg:block py-0.5">all in one place</span>
            </h2>
          ...
          {/* Right Column (60% width on lg) - Dynamic Orbital Visual */}
          <div className="lg:col-span-7 flex justify-center items-center relative min-h-[300px] sm:min-h-[500px] md:min-h-[620px] select-none overflow-hidden mt-10 sm:mt-10 lg:mt-0 mb-10 sm:mb-0 z-10">

            {/* Scaled Orbital Wrapper for perfect mobile scaling */}
            <div className="relative w-[340px] h-[340px] sm:w-[500px] sm:h-[500px] md:w-[560px] md:h-[560px] flex items-center justify-center scale-[0.70] min-[380px]:scale-78 sm:scale-100 transition-transform duration-300">

              {/* Center Logo Node */}
              <div className="relative z-20 flex items-center justify-center w-20 h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full bg-white border-4 border-white shadow-[0_0_60px_rgba(255,255,255,0.5)] backdrop-blur-xl group hover:scale-110 transition-transform duration-500">
                <img
                  src={lettzoLogo}
                  alt="Lettzo"
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                ...
                {/* Ring 1 (Inner) */}
                <div className="absolute w-[200px] h-[200px] sm:w-[260px] sm:h-[260px] rounded-full border-2 border-white/30 animate-[spin_26s_linear_infinite] hover:[animation-play-state:paused] pointer-events-auto">
                  {ring1.map((item, idx) => {
                ...
                {/* Ring 2 (Middle) */}
                <div className="absolute w-[310px] h-[310px] sm:w-[400px] sm:h-[400px] rounded-full border-2 border-white/30 border-dashed animate-[spin_32s_linear_infinite_reverse] hover:[animation-play-state:paused] pointer-events-auto">
                  {ring2.map((item, idx) => {
                ...
                {/* Ring 3 (Outer) */}
                <div className="absolute w-[420px] h-[420px] sm:w-[540px] sm:h-[540px] rounded-full border-2 border-white/20 animate-[spin_38s_linear_infinite] hover:[animation-play-state:paused] pointer-events-auto">
                  {ring3.map((item, idx) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl ${item.bg} border ${item.border} backdrop-blur-md shadow-xs flex items-center justify-center transition-all duration-300 ${isActive
            ? "scale-125 border-primary ring-4 ring-primary/20 shadow-xl bg-white z-30"
            : "group-hover:scale-115 group-hover:shadow-md"
            }`}
        >
          <IconComponent className={`w-4 h-4 sm:w-6 sm:h-6 ${item.color} transition-transform duration-300 group-hover:scale-110`} />
        </div>

        {/* Hover/Tap Label Pill */}
        <div
          className={`absolute left-1/2 -translate-x-1/2 top-full mt-1.5 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-slate-900 text-white text-[10px] sm:text-[11px] font-bold whitespace-nowrap shadow-lg transition-all duration-200 pointer-events-none z-40 ${isActive
